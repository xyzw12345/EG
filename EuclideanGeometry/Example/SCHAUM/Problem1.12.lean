...
structure Setting (Plane : Type _) [EuclideanPlane Plane] where
  ...
  C_ne_B : C ≠ B := ABCD_IsND.nd₂₃
  A_ne_D : A ≠ D := (ABCD_IsND.nd₁₄).symm
  -- Assume that the diagonal $BD \perp BC$ and $BD \perp DA$.
  BD_perp_BC : (SEG_nd B D D_ne_B.out) ⟂ (SEG_nd B C C_ne_B)
  BD_perp_DA : (SEG_nd B D D_ne_B.out) ⟂ (SEG_nd D A A_ne_D)
  -- Suppose that $BC = DA$.
